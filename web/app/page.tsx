    ...
    <main className="min-h-screen bg-slate-100 p-4 md:p-8">
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex justify-between items-center">
          ...
          <form action={logout}>
            <button className="text-sm text-slate-500 hover:text-slate-700 font-medium border border-slate-300 px-4 py-2 rounded-lg transition hover:bg-slate-50">
              Sair do Sistema
            </button>
          </form>
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1">
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 h-fit sticky top-4">
              <h2 className="text-base font-bold mb-4 text-slate-800 border-b border-slate-100 pb-3">
                Novo Chamado
              </h2>
          ...
          <div className="lg:col-span-2">
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
              <h2 className="text-base font-bold mb-4 text-slate-800 border-b border-slate-100 pb-3 flex justify-between items-center">
                <span>Meus Chamados Recentes</span>
                <span className="text-xs font-medium text-slate-400">Últimos 10</span>
              </h2>
              {myTickets.length === 0 ? (
                ...
                <div className="space-y-4">
                  {myTickets.map((ticket) => (
                    <div key={ticket.id} className="border border-slate-200 rounded-xl p-4 hover:shadow-md transition bg-slate-50/50">
                        ...
                        {ticket.status === 'IN_PROGRESS' && (
                          <div className="w-full">
                            <div className="flex items-center gap-2 mb-2">
                              <span className="bg-indigo-100 text-indigo-700 text-xs font-bold px-3 py-1 rounded-full flex items-center gap-2">
                                🔵 Em Análise pela TI
                              </span>
                              <span className="text-xs text-slate-400">Não precisa mandar Zap!</span>
                            </div>

                            {/* --- DATA AGENDADA --- */}
                            {ticket.estimatedTime && (
                                <div className="mb-2 text-xs font-bold text-indigo-800 bg-indigo-50 border border-indigo-200 p-1 px-2 rounded w-fit flex items-center gap-1">
                                    📅 Agendado para: {ticket.estimatedTime.toLocaleString('pt-BR')}
                                </div>
                            )}
                            {/* Mensagem da TI */}
                            {ticket.tiResponse && (
                              <div className="bg-indigo-50 border border-indigo-100 p-2 rounded text-xs text-indigo-800 italic flex gap-2 items-start">
                                <span>💬</span>
